...
class DownloadService:
    ...
    def download_card(user_id, card_id):
        # DEPRECATED — kept only so the existing POST /api/cards/<id>/download
        # route doesn't 404 if anything old still calls it. This never
        # composited the saved customization onto the image (see the
        # "Stand-in" comment a few lines down) — it just returns the blank
        # template's URL. The dashboard (Purchases.jsx) used to call this
        # and is why downloads from there had no text on them; it has been
        # switched to get_downloadable_file() below, which does render the
        # customization properly. Don't wire any new UI to this function —
        # use get_downloadable_file() instead.
    ...
    def get_downloadable_file(user_id, card_id, fmt="image"):
        ...
        if fmt == "gif":
            ...
            clean_path = card.animated_gif.lstrip("/")
            if clean_path.startswith("static/"):
                gif_relative = clean_path.replace("static/", "", 1)
            else:
                gif_relative = clean_path
        ...
        # --- Image / PDF path ---
        # Un-indented below so it actually runs when fmt is not "gif"
        file_path = card.thumbnail
        if card.templates:
            file_path = card.templates[0].preview_image

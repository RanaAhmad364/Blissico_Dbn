...
class DownloadService:
    ...
    def get_downloadable_file(user_id, card_id, fmt="image"):
        ...
        if fmt == "gif":
            ...
            db.session.add(Download(user_id=user_id, card_id=card_id, downloaded_at=datetime.now(timezone.utc), file_path=card.animated_gif))
            db.session.commit()
        ...
        db.session.add(Download(user_id=user_id, card_id=card_id, downloaded_at=datetime.now(timezone.utc), file_path=file_path))
        db.session.commit()
        return out_bytes, filename, mimetype
